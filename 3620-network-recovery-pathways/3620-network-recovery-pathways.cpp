...
public:
    ...
    bool canAchieve(int node, int limit, long long budget,
    ...
    long long dfs(int node, int limit, vector<vector<pair<int, int>>>& graph,
                  vector<long long>& dp) {
        ...
        for (auto& [next, weight] : graph[node]) {
            ...
            if (weight < limit)
                continue;
            bestCost = min(bestCost, weight + dfs(next, limit, graph, dp));
        }
